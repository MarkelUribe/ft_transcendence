<script lang="ts">
	...
	$: display =
		myColor === "b" ? board.map((r) => [...r].reverse()).reverse() : board;
	$: isReviewMode =
	...
	onMount(async () => {
		...
		socket.on("chatHistory", (history) => {
			gameMessages = Array.isArray(history) ? history : [];
		});
	...
	function getTurnText(
		...
		if (isReviewMode) {
			if (currentMoveIndex == 0) return "Start of the Match";
					...
					<div class="player-chip-row">
						<a class="player-chip" href={`/profile/${blackId}`}>
							...
							<div class="player-name_elo">
								<span class="player-chip__name"
									>{blackPlayer?.username}</span
								>
								<span class="player-chip__elo"
									>{blackPlayer?.elo ?? 0} ELO</span
								>
							</div>
						</a>
					</div>
				...
				{:else}
					<div class="player-chip-row">
						<a class="player-chip" href={`/profile/${whiteId}`}>
							...
							<div class="player-name_elo">
								<span class="player-chip__name"
									>{whitePlayer?.username}</span
								>
								<span class="player-chip__elo"
									>{whitePlayer?.elo ?? 0} ELO</span
								>
							</div>
						</a>
					</div>
				...
				<div class="chess-board">
					{#each display as row, r}
						{#each row as cell, c}
							<button
								type="button"
								class="square {(r + c) % 2 === 0
									? 'light'
									: 'dark'}"
								class:selected={selected ===
									coordFromDisplay(r, c)}
								class:move-highlight={coordFromDisplay(r, c) ===
									moveFrom ||
