<script lang="ts">
  ...
  $: display =
    myColor === "black" ? board.map((r) => [...r].reverse()).reverse() : board;
  $: isReviewMode =
    currentMoveIndex != logs.length - 1 || gameStatus === "ended";
  ...
  function parseFen(fen: string) {
    return fen
      ...
      .map((row) => {
        ...
        for (const ch of row) {
          if (/\d/.test(ch)) result.push(...Array(Number(ch)).fill(null));
          else result.push(ch);
        }
        return result;
      });
  ...
  function updateDrawOffer(msg: any) {
    if (
      (msg.whiteDraw && myColor === "black") ||
      (msg.blackDraw && myColor === "white")
    )
      drawOffer = true;
    else drawOffer = false;
  }
  ...
  function handleEnd(msg: any) {
    ...
    if (!myColor) resultText = "Match ended";
    else if (msg.looser === -1) resultText = "Draw";
    else resultText = msg.looser === myId ? "Defeat" : "Victory";

    gameStatus = "ended";
    gameOver = true;
  }

  function originalIndices(r: number, c: number) {
    return myColor === "black" ? [7 - r, 7 - c] : [r, c];
  }
  function getPieceImage(piece: string | null) {
    return piece ? `/pieces/${piece}.png` : "";
  }
  function goHome() {
  ...
  onMount(async () => {
    ...
    socket.on("connect", () => {
      socket.emit("joinGame", { gameId, playerId: localStorage.getItem("id") });
    });
    ...
    <div class="top-bar">
      <div class="spacer">
        {#if myColor !== "black"}
          ...
          <div
            class="bg-secondary border rounded-3 px-4 py-2 d-inline-block m-3 shadow-sm"
            class:bg-danger={blackClock < 16000}
          >
        ...
        {:else}
          ...
          <div
            class="clock-badge bg-secondary border rounded-3 px-4 py-2 d-inline-block m-3 shadow-sm"
            class:bg-danger={whiteClock < 15000}
          >
        ...
        {#if promotion && myColor}
          <div
            class="d-flex gap-1 p-1 rounded-2 shadow-sm border bg-light bg-opacity-25 backdrop-blur align-items-center"
          >
        ...
        {:else if drawOffer}
          <div class="controls">
            <button
              ...
              on:click={() => {
                showDraw = true;
                headerText = "Accept the Draw?";
                bodyText = "The game will end in a draw if accepted.";
              }}
            >
              Accept Draw?
            </button>
          ...
          {#each display as row, r}
            {#each row as cell, c}
              <button
                ...
                class:move-highlight={coordFromDisplay(r, c) === moveFrom ||
    ...
    <div class="bottom-bar">
      <div class="spacer">
        {#if myColor !== "white"}
          ...
          <div
            class="bg-secondary border rounded-3 px-4 py-2 d-inline-block m-3 shadow-sm"
            class:bg-danger={blackClock < 15000}
          >
            {formatTime(blackClock)}
          </div>
        {:else}
        ...
        {#if !isReviewMode}
          <div class="controls">
            <button
              ...
              on:click={() => {
                showSurrender = true;
                headerText = "Confirm Surrender";
                bodyText = "You will lose the game.";
              }}
            >
              🏳️
            </button>
      ...
      <div class="move-controls-panel">
        <button
          class="nav-btn"
          on:click={goToStart}
          disabled={currentMoveIndex <= 0}>◀◀</button
        >
        <button
          class="nav-btn"
          on:click={prevMove}
        ...
        <button
          class="nav-btn"
          on:click={nextMove}
          disabled={currentMoveIndex >= logs.length - 1}>▶</button
        >
        <button
          class="nav-btn"
          on:click={goToEnd}
